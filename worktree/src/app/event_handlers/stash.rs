            ...
            KeyCode::Char('a') => {
                if let Some(stash) = stashes.get(*selected_index) {
                    if let Err(e) = repo.apply_stash(path, stash.index) {
                        return Some(AppState::Error(format!("Failed to apply stash: {e}"), Box::new(_current_state.clone())));
                    }
            ...
            KeyCode::Char('p') => {
                if let Some(stash) = stashes.get(*selected_index) {
                    if let Err(e) = repo.pop_stash(path, stash.index) {
                        return Some(AppState::Error(format!("Failed to pop stash: {e}"), Box::new(_current_state.clone())));
                    }
                    return Some(*prev_state.clone());
                }
            }
            KeyCode::Char('d') => {
                if let Some(stash) = stashes.get(*selected_index) {
                    if let Err(e) = repo.drop_stash(path, stash.index) {
                        return Some(AppState::Error(format!("Failed to drop stash: {e}"), Box::new(_current_state.clone())));
                    }
                    if let Ok(new_stashes) = repo.list_stashes(path) {
                        let new_index = selected_index.min(&new_stashes.len().saturating_sub(1)).clone();
                        return Some(AppState::ViewingStashes {
                            path: path.to_string(),
                            branch: branch.to_string(),
                            stashes: new_stashes,
