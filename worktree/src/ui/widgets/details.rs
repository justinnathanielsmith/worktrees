...
impl Widget for DetailsWidget<'_> {
    fn render(self, area: Rect, buf: &mut ratatui::buffer::Buffer) {
        ...
        let details_text = self.worktree.map_or_else(
            || {
                vec![
                    Line::from(""),
            ...
            |wt| {
                ...
                if wt.is_bare {
                    ...
                    let dirty_count = active_wt
                        .iter()
                        .filter(|w| w.status_summary.as_ref().is_some_and(|s| s != "clean"))
                        .count();
                    ...
                    lines.push(Line::from(vec![
                        ...
                        Span::styled(format!("{clean_count}"), Style::default().fg(theme.success)),
                    ]));
                    lines.push(Line::from(vec![
