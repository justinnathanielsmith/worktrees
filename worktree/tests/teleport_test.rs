...
async fn test_teleport_changes_between_worktrees() {
    ...
    repo.add_new_worktree("feat-a", "feat-a", "main")
        .expect("Failed to create feat-a");

    // Create current worktree 'dev'
    repo.add_new_worktree("dev", "dev", "main")
    ...
    repo.stage_all(&dev_path.to_string_lossy())
        .expect("Failed to stage changes");

    // Verify changes are in 'dev'
    let status_dev = repo
        .get_status(&dev_path.to_string_lossy())
    ...
    let status_feat_a = repo
        .get_status(&feat_a_path.to_string_lossy())
        .expect("Failed to get status");

    // Note: git stash apply may restore changes to either staged or unstaged depending on source state and git version.
    let found_staged = status_feat_a
        .staged
        .iter()
        .any(|(f, _)| f == "teleport_test.txt");
    let found_unstaged = status_feat_a
        .unstaged
        .iter()
        .any(|(f, _)| f == "teleport_test.txt");
    assert!(
        found_staged || found_unstaged,
        "File 'teleport_test.txt' not found in target worktree status. Status: {:?}",
        status_feat_a
    );

    let content = fs::read_to_string(feat_a_path.join("teleport_test.txt"))
        .expect("Failed to read file in target");
